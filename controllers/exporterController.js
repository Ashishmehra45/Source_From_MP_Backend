...
const registerSeller = async (req, res) => {
  try {
    // ✅ 1. req.body se address, city aur state bhi nikal lo
    const {
      companyName,
      authorizedPerson,
      mobileNumber,
    ...
    // User exists check (Same as before)
    const userExists = await Exporter.findOne({ email });
    if (userExists)
      return res.status(400).json({ message: "Exporter already exists" });
    ...
    // ✅ 2. Exporter create karte waqt nayi fields save kar do
    const exporter = await Exporter.create({
      ...
      iecNumber,
      exportCountries,
      address, // Database mein save hoga
      city, // Database mein save hoga
      state, // Database mein save hoga
  ...
  try {
    ...
    if (!req.user || !req.user._id) {
      ...
      return res.status(401).json({
        message: "User authentication failed. Token might be missing.",
      });
    }
...
const getPublicProducts = async (req, res) => {
  try {
    // ✅ Seller field ko populate kiya aur sirf 'city' aur 'companyName' uthaya
    const products = await Product.find({})
      .populate("seller", "city companyName") // <-- YE LINE ADD KI HAI
      .sort({ createdAt: -1 })
      .limit(8);
...
const getBuyerEnquiries = async (req, res) => {
  try {
    const inquiries = await Inquiry.find({ sellerId: req.user._id })
      .populate({
        path: "buyerId",
        model: "Buyer", // 👈 Yahan exactly "Buyer" likho
        // ✅ Address aur mobileNumber hata diya kyunki wo schema mein nahi hain
        select: "name email companyName city state country",
      })
      .populate({
        path: "productId",
        model: "Product",
    ...
    res.status(500).json({ message: error.message }); // Frontend pe exact error dikhegi
  }
};
...
const updateInquiryStatus = async (req, res) => {
  try {
    ...
    if (!updatedInquiry) {
      return res
        .status(404)
        .json({ success: false, message: "Inquiry nahi mili!" });
    }
    ...
    res.status(500).json({
      success: false,
      message: "Server error status update karne mein",
    });
  }
};
